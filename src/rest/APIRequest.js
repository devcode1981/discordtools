...
class APIRequest {
    constructor(apimanager, method, path, auth, data, files, reason) {
        ...
        this.data = data;
        this.files = files;
        this.route = this.fetchRoute(this.path);
        this.reason = reason;
    }
    ...
    fetchAuth() {
        ...
        throw new Error(Constants.Errors.EMPTY_TOKEN);
    }

    generate() {
        const API = `${this.client.options.http.host}/api/v${this.client.options.http.version}`;
        const request = snekfetch[this.method](`${API}${this.path}`);
        if (this.auth) request.set('Authorization', this.fetchAuth());
